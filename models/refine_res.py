...
@Date: 2019-03-22 14:18:07
@LastEditTime: 2019-03-22 18:30:51
'''
# -*- coding: utf-8 -*-
# Written by yq_yao
...
def add_extras(size, in_channel, batch_norm=False):
    ...
    layers = []
    layers += [nn.Conv2d(in_channel, 64, kernel_size=1, stride=1)]
    layers += [nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1)]
    return layers
...
class BottleneckForRes18(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(BottleneckForRes18, self).__init__()
...
class RefineForResnet18(nn.Module):
    def __init__(self, block, num_blocks, size):
...
def RefineResnet18(size, channel_size='48'):
    return RefineForResnet18(BottleneckForRes18, [2, 2, 2, 2], size)
...
if __name__ == "__main__":
    ...
    model = RefineResnet18(size=1024)
    print(model)
    # with torch.no_grad():
    #     model.eval()
